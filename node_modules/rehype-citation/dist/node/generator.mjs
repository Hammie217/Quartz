...
const getBibliography = async (options, file) => {
  ...
  if (options.bibliography) {
    ...
    bibliography = typeof frontmatterBibliography === 'string' ? [frontmatterBibliography] : frontmatterBibliography;
  }
...
const getFrontmatterField = (file, fieldName) => {
  var _file$data$astro;
  if (!file || !file.data) {
    return undefined;
  }
  const sources = [file.data.matter, file.data.frontmatter, (_file$data$astro = file.data.astro) == null ? void 0 : _file$data$astro.frontmatter];
...
const rehypeCitationGenerator = Cite => {
  return (options = {}) => {
    return async (tree, file) => {
      var _options$inlineBibCla;
      /** @type {string[]} */
      let bibtexFile = [];
      const inputCiteformat = /** @type {string} */
      options.csl || getFrontmatterField(file, 'csl') || defaultCiteFormat;
      const noCite = /**  @type {string[] | false} */
      ...
      if (noCite) {
        if (noCite.length === 1 && noCite[0] === '@*') {
          ...
          const mergedIds = citations.data.filter(x => noCite.map(x => x.replace('@', '')).includes(x['citation-key'])).map(x => x.id).concat(parsedEntries.map(x => x.id));
